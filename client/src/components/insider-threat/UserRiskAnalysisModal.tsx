...
export default function UserRiskAnalysisModal({
  ...
  return (
    ...
    >
      <div
        className="bg-[#1f1f1f] rounded-2xl w-[85%] max-w-[700px] max-h-[80%]! overflow-auto 
      ...
      >
        <div className="flex justify-between  items-center px-6 py-5 border-b border-[#333] bg-[#2a2a2a] rounded-t-2xl">
          <button
            ...
            className="flex items-center! bg-[#3a3a3a] hover:bg-[#4a4a4a] h-10! w-[19%]! text-white px-4 py-2 rounded-lg text-sm transition disabled:opacity-50 border-none cursor-pointer"
          >
            <PiArrowCounterClockwiseFill size={20} />
            {recalculating ? "Recalculating..." : "Recalculate"}
          </button>

          <h2 className="w-1/3! ml-6! text-xl text-white">User Risk Analysis</h2>
        ...
        <div className="flex flex-col px-6! py-6! gap-6! text-sm">
          ...
          <div>
            <label className="block text-gray-400 mb-2! text-base">Behavior Patterns</label>
            <div className="grid grid-cols-2 gap-6">
              {Object.entries(analysis.behaviorPatterns).map(([key, value]) => (
                <div key={key} className="bg-[#2a2a2a] rounded-lg px-2! py-1! border border-[#3a3a3a]">
          ...
          {analysis.recentThreats && analysis.recentThreats.length > 0 && (
            <div>
              <label className="block text-gray-400 mb-2! text-base">Recent Threats ({analysis.recentThreats.length})</label>
              <div className="space-y-5!">
                {analysis.recentThreats.map((threat) => (
                  <div key={threat.id} className="bg-[#2a2a2a] rounded-lg px-2! py-3! border border-[#3a3a3a]">
                    <div className="text-white  text-sm">{threat.threatType.replace(/_/g, " ")}</div>
                    <div className="text-white leading-relaxed text-sm mt-2">{threat.description}</div>
                    <div className="flex items-center gap-2 mt-3">
                      <span className="text-white text-sm">RISK LEVEL:</span>
                      <span className={`text-xs font-semibold px-3 py-1 rounded ${
                        threat.riskLevel === 'CRITICAL' ? ' text-red-400' :
                        threat.riskLevel === 'HIGH' ? 'text-orange-400' :
                        threat.riskLevel === 'MEDIUM' ? ' text-yellow-400' :
                        'text-green-400'
                      }`}>
                        {threat.riskLevel}
                      </span>
          ...
          <div className="bg-blue-900/20 border-l-4 border-blue-500 rounded-lg px-2! py-3!">
